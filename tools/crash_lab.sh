...
MEM_LIMITS=${MEM_LIMITS:-1500m 4g}
IMG_LENGTH_LIMITS=${IMG_LENGTH_LIMITS:-640 0}
    ...
    for img_length_limit in ${IMG_LENGTH_LIMITS/,/ }; do
      for img_name in ${IMG_NAMES/,/ }; do
        ...
        OUTPUT=$(docker run --memory=$mem_limit --memory-swap=$mem_limit -e IMG_LENGTH_LIMIT=$img_length_limit \
          -e SCANNER=$scanner -e IMG_NAME=$img_name "frs-core_ml${ID}" python -m src.services.facescan.run 2>&1)
        EXIT_CODE=$?
