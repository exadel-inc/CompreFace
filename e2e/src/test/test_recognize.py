...
def test__when_retraining__then_returns_400():
    ...
    res = ml_post("/retrain", headers={'X-Api-Key': ENV.API_KEY})
...
def test__given_img_with_no_faces__when_adding_face__then_returns_400_no_face_found():
    ...
    res = ml_post("/faces/JoeSmith", headers={'X-Api-Key': ENV.API_KEY}, files=files)
...
def test__when_adding_face__then_returns_201(file, name):
    ...
    res = ml_post(f"/faces/{name}?retrain=no", headers={'X-Api-Key': ENV.API_KEY}, files=files)
...
def test__when_retraining__then_returns_202():
    ...
    ml_wait_until_training_is_completed(ENV.API_KEY)
...
def test__when_recognizing_faces__then_returns_face_A_name():
    ...
    res = ml_post("/recognize", headers={'X-Api-Key': ENV.API_KEY}, files=files)
...
def test__given_five_face_img__when_recognizing_faces__then_returns_five_distinct_results():
    ...
    res = ml_post("/recognize", headers={'X-Api-Key': ENV.API_KEY}, files=file)
...
def test__when_getting_names__then_returns_correct_names():
    ...
    res = ml_get("/faces", headers={'X-Api-Key': ENV.API_KEY})
...
def test__given_other_api_key__when_getting_names__then_returns_no_names():
    ...
    res = ml_get("/faces", headers={'X-Api-Key': f'{ENV.API_KEY}-different'})
...
def test__when_deleting_face__then_returns_204():
    ...
    res_del = ml_delete("/faces/Paul Walker", headers={'X-Api-Key': ENV.API_KEY})
...
def test__when_recognizing_faces__then_only_faces_A_and_B_are_recognized():
    ...
    res_a = ml_post("/recognize", headers={'X-Api-Key': ENV.API_KEY}, files=files_a)
    res_b = ml_post("/recognize", headers={'X-Api-Key': ENV.API_KEY}, files=files_b)
    res_c = ml_post("/recognize", headers={'X-Api-Key': ENV.API_KEY}, files=files_c)
    ...
    ml_wait_until_training_is_completed(ENV.API_KEY, check_response=False)
...
def test__when_getting_training_status__then_returns_200():
    ...
    res = ml_get("/retrain", headers={'X-Api-Key': ENV.API_KEY})
...
def test__when_deleting_face_B_with_retraining__then_returns_204():
    ...
    res = ml_delete("/faces/Stephen Hawking", headers={'X-Api-Key': ENV.API_KEY})
