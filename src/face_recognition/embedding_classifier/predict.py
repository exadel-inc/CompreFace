...
def predict_from_embedding(embedding, face_box: BoundingBox, api_key: ApiKey) -> FacePrediction:
    ...
    return FacePrediction(face_name=model.class_2_face_name[top_class],
                          probability=probabilities[top_class],
                          box=face_box)


def predict_from_image(img, limit: Union[FaceLimitConstant, int], api_key: ApiKey) -> List[FacePrediction]:
