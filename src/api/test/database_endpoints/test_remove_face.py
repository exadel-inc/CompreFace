from http import HTTPStatus

from src.api.constants import API_KEY_HEADER

FILE_BYTES = b''


def test__when_remove_face_is_requested__then_removes_the_face(client, mocker):
    get_database_mock = mocker.Mock()
    mocker.patch('src.api.controller.get_database', return_value=get_database_mock)

    res = client.delete('/faces/John Smith', headers={API_KEY_HEADER: 'api-key-001'})

    get_database_mock.remove_face.assert_called_once_with('api-key-001', "John Smith")
    assert res.status_code == HTTPStatus.NO_CONTENT
